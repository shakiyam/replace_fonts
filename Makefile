...
all: check_for_updates lint update_requirements_dev build_dev mypy pytest update_requirements build test ## Check for updates, lint, update requirements.txt, build, and test
...
check_for_image_updates: ## Check for image updates
	@echo -e "\033[36m$@\033[0m"
	@./tools/check_for_image_updates.sh "$$(awk '/^FROM /{print $$2; exit}' Dockerfile)" docker.io/library/python:slim
	@./tools/check_for_image_updates.sh "$$(awk '/COPY --from=.*astral-sh\/uv/{sub(/.*--from=/,""); print $$1; exit}' Dockerfile)" ghcr.io/astral-sh/uv:latest
